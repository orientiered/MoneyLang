...
static bool needBrackets(Node_t *node) {
    ...
    if (node->type == OPERATOR) {
        ...
        return (operators[node->parent->value.op].opCode == OP_POW || parentPriority > currentPriority);
    }
    return false;
}


static void writeAsProgramRecursive(LangContext_t *context, FILE *file, Node_t *node) {
    if (!node) return;
    ...
    if (!binary) {
        ...
        writeAsProgramRecursive(context, file, node->left);
        ...
        writeAsProgramRecursive(context, file, node->left);
        fprintf(file, " %s ", operators[node->value.op].str);
        if (node->value.op == OP_SEMICOLON)
            fprintf(file, "\n");
        writeAsProgramRecursive(context, file, node->right);
...
FrontendStatus_t writeAsProgram(LangContext_t *context) {
    ...
    writeAsProgramRecursive(context, file, context->tree);
